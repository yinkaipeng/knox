...
public class SecureKnoxShellTest {
  ...
  /* Kerberos client */
  private static KrbClient krbClient;
  ...
  /**
   * Kinit using the given principal and keytab  and store the ticket on disk to
   * be used by KnoxShell
   *
   * @throws KrbException
   * @throws IOException
   */
  public static void kinit(final String principal, final String keytab)
      throws Exception {
    /* login using  kerberos client and store the ticket in ticketCache file  */
    krbClient = new KrbClient(((MiniKdc) kdc).getKrb5conf().getParentFile());
    krbClient.init();
    final TgtTicket tgt = krbClient.requestTgt(principal, new File(keytab));

    /* cleanup older cache  */
    Files.deleteIfExists(ticketCache.toPath());
    krbClient.storeTicket(tgt, ticketCache);
  }
  ...
  public void testCachedTicket() throws Exception {
    ...
    /*
    kinit(hdfsPrincipal, keytab);
    assertTrue("Cached principal does not same as the one used in kinit",
        krbClient.getCredentialFromFile(ticketCache).getClientName().toString()
            .equalsIgnoreCase(hdfsPrincipal));
    */
